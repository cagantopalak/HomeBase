...
function getMainTileHex() {
  ...
  if (typeof savedTileColor === "string" && savedTileColor.startsWith("#"))
    return savedTileColor;
  // Otherwise assume rgba(...) and convert
  try {
    return rgbaToHex(savedTileColor);
  } catch (e) {
    return "#ffffff";
  }
}
...
function applyTilePlacement() {
  ...
  const placement =
    pendingTilePlacement !== null && typeof pendingTilePlacement !== "undefined"
...
if (savedTileColor) {
  ...
  document.documentElement.style.setProperty(
    "--tile-border-color",
    savedTileColor.replace("0.4)", "0.5)")
  );
} else {
  document.documentElement.style.setProperty(
    "--tile-bg-color",
...
function updateRangeBackground(el) {
  ...
  const trackColor =
    style.getPropertyValue("--input-border-dark") || "rgba(255,255,255,0.2)";
...
function setupSoundSlider() {
  ...
  volumeSlider.addEventListener("input", (e) => {
    ...
    if (sliderValue <= -1) {
      // If slider is at -1 or less, actual audio volume is 0
      currentVolume = 0;
...
/* ---------- TILE MANAGEMENT ---------- */
let links = [];
// stickyNotes array is now in sticky-notes.js

// If running as an extension with browser.storage, use that
if (typeof browser !== "undefined" && browser.storage) {
  browser.storage.local
    .get(["tiles", "stickyNotes"])
    .then((result) => {
...
function buildTile(link, index, parentLinks = links) {
  ...
  if (parentLinks === links) {
    tile.addEventListener("dragstart", function (e) {
      const idx =
        typeof selfIndex !== "undefined" ? selfIndex : +this.dataset.index;
      dragStartIndex = idx;
    ...
    tile.addEventListener("dragover", function (e) {
      ...
      const targetIsFolder =
        links[selfIndex] && links[selfIndex].type === "folder";
...
function buildFolderTile(folder, index) {
  ...
  folder.links.slice(0, 4).forEach((link) => {
    ...
    if (folder.colorHex) {
      ...
      iconDiv.style.borderColor = hexToRgbaAlpha(
        folder.colorHex,
        TILE_BORDER_ALPHA
      );
    } else {
  ...
  tile.addEventListener("dragover", function (e) {
    ...
    const isDraggingFile =
      !dragFromFolder &&
      links[dragStartIndex] &&
      links[dragStartIndex].type !== "folder";
...
function dragOver(e) {
  ...
  document
    ...
    .forEach((tile) => tile.classList.remove("moving"));
...
function openFolder(folder, index) {
  ...
  if (folder.colorHex) {
    innerBubble.style.background = hexToRgbaAlpha(
      folder.colorHex,
      TILE_BG_ALPHA
    );
    innerBubble.style.borderColor = hexToRgbaAlpha(
      folder.colorHex,
      TILE_BORDER_ALPHA
    );
  ...
  folder.links.forEach((link, i) => {
    ...
    if (innerTile) {
      ...
      if (folder.colorHex) {
        innerTile.style.background = hexToRgbaAlpha(
          folder.colorHex,
          TILE_BG_ALPHA
        );
        innerTile.style.borderColor = hexToRgbaAlpha(
          folder.colorHex,
          TILE_BORDER_ALPHA
        );
      } else {
    ...
    innerTiles.forEach((el) => {
      ...
      if (folder.colorHex) {
        ...
        el.style.borderColor = hexToRgbaAlpha(
          folder.colorHex,
          TILE_BORDER_ALPHA
        );
      } else {
...
function openFolderEditModal(folder, index) {
  ...
  if (folder.colorHex) {
    ...
    if (resetFolderColorBtn)
      resetFolderColorBtn.classList.remove("active-reset");
  } else {
    // show main tile color so user sees what resetting will do
    const mainHex = getMainTileHex();
    folderColorInput.value =
      mainHex && mainHex.length === 7 ? mainHex : "#ffffff";
    folderColorInput.dataset.useDefault = "1";
...
const resetAllSettingsAndLinksBtn = document.getElementById(
  "resetAllSettingsAndLinksBtn"
);
const resetAllSettingsOnlyBtn = document.getElementById(
...
const exportAllSettingsAndLinksBtn = document.getElementById(
  "exportAllSettingsAndLinksBtn"
);
const exportAllSettingsOnlyBtn = document.getElementById(
  "exportAllSettingsOnlyBtn"
);
const exportAllLinksOnlyBtn = document.getElementById("exportAllLinksOnlyBtn");
const cancelExportOptionsBtn = document.getElementById(
  "cancelExportOptionsBtn"
);

let savedShowClock =
  localStorage.getItem("showClock") === "false" ? false : true;
let savedClockColor = localStorage.getItem("clockColor") || "#FFFFFF";
let savedClockFontFamily =
  localStorage.getItem("clockFontFamily") || "'Climate Crisis', cursive";
let savedClockFormat = localStorage.getItem("clockFormat") || "24";
let savedShowSeconds =
  localStorage.getItem("showSeconds") === "true" ? true : false;
...
function applyClockSettings() {
  ...
  document.documentElement.style.setProperty(
    "--clock-display-font-family",
    savedClockFontFamily
  );
  // apply size
  document.documentElement.style.setProperty(
    "--clock-font-size",
    savedClockSize ? savedClockSize + "px" : "4em"
  );

  // apply position via helper classes
  if (newDigitalClock) {
    newDigitalClock.classList.remove("clock-pos-left", "clock-pos-right");
    if (savedClockPosition === "right")
  ...
  const showClockNow =
    pendingShowClock !== null
      ? pendingShowClock
      : localStorage.getItem("showClock") !== "false"
        ? true
        : false;
...
function openEditModal() {
  ...
  const savedTileColorModal =
    localStorage.getItem("tileColor") || "rgba(255,255,255,0.4)";
...
function closeEditModal() {
  ...
  document.documentElement.style.setProperty(
    "--tile-label-font-family",
    localStorage.getItem("fontFamily") || "'Roboto', sans-serif"
  );
  const savedTileColorOnCancel =
    localStorage.getItem("tileColor") || "rgba(255,255,255,0.4)";
  document.documentElement.style.setProperty(
    "--tile-bg-color",
    savedTileColorOnCancel
  );
  document.documentElement.style.setProperty(
    "--tile-border-color",
    savedTileColorOnCancel.replace("0.4)", "0.5)")
  );
  ...
  currentVolume =
    isNaN(savedSliderValue) || savedSliderValue < 0 ? 0 : savedSliderValue;
  if (currentVolume === 0 && audioCtx.state === "running") {
    audioCtx
      .suspend()
      .catch((e) => console.error("Suspend failed on close:", e));
  ...
  const savedSliderValueOnClose = parseFloat(
    localStorage.getItem("soundVolume")
  );
  currentVolume =
    isNaN(savedSliderValueOnClose) || savedSliderValueOnClose < 0
      ? 0
      : savedSliderValueOnClose;
  ...
  reader.onload = async function (event) {
    try {
      ...
      try {
        ...
        try {
          sessionStorage.setItem("sessionCustomBackground", dataUrl);
        } catch (e) { }
      }
    } catch (err) {
...
const genericModalConfirmBtn = document.getElementById(
  "genericModalConfirmBtn"
);
...
function clearAllFolderInlineStylesIfNoOverride() {
  document.querySelectorAll(".tile.folder-tile").forEach((el) => {
    const idx =
      el.dataset.index !== undefined ? parseInt(el.dataset.index, 10) : NaN;
...
if (folderColorInput) {
  folderColorInput.addEventListener("input", (e) => {
    ...
    if (
      activeFolder &&
      folderEditIndex !== null &&
      activeFolder.index === folderEditIndex
    ) {
      const inner = folderBubble.querySelector(".folder-bubble-inner");
      if (inner) {
...
function setupCustomDropdown(dropdownId, hiddenInputId, onChangeCallback) {
  ...
  const initialOption = Array.from(options).find(
    (o) => o.dataset.value === initialValue
  );
  if (initialOption) {
...
saveSettingsBtn.addEventListener("click", () => {
  ...
  localStorage.setItem(
    "tileColor",
    document.documentElement.style.getPropertyValue("--tile-bg-color")
  );
  ...
  const placementToSave =
    tilePlacementSelect && tilePlacementSelect.value
      ? tilePlacementSelect.value
      : pendingTilePlacement || "top";
  ...
  const showClockToSave =
    typeof pendingShowClock === "boolean"
      ? pendingShowClock
      : showClockToggle.checked;
  ...
  currentVolume =
    isNaN(savedSliderAfterSave) || savedSliderAfterSave < 0
      ? 0
      : savedSliderAfterSave;
  if (currentVolume === 0 && audioCtx.state === "running") {
    audioCtx
      .suspend()
      .catch((e) => console.error("Suspend failed after save:", e));
  } else if (currentVolume > 0 && audioCtx.state === "suspended") {
    audioCtx
      .resume()
      .catch((e) => console.error("Resume failed after save:", e));
  }
    ...
    async () => {
      if (typeof browser !== "undefined" && browser.storage) {
        try {
          ...
          console.error(
            "Error removing background from browser.storage:",
            error
          );
    ...
    () => {
      ...
      if (typeof browser !== "undefined" && browser.storage) {
        browser.storage.local.remove(["tiles", "stickyNotes"]).then(() => {
          showCustomAlert(
            "All links and notes have been reset! The page will now reload."
          );
    ...
    () => {
      ...
      if (typeof browser !== "undefined" && browser.storage) {
        browser.storage.local.clear().then(() => {
          showCustomAlert(
            "All settings and tiles have been reset! The page will now reload."
          );
          setTimeout(() => location.reload(), 1500);
        });
      } else {
    ...
    async () => {
      ...
      const browserTiles =
        typeof browser !== "undefined" && browser.storage
          ? browser.storage.local.get("tiles")
          : Promise.resolve({ tiles: null });
      ...
      try {
        ...
        showCustomAlert(
          "All settings have been reset! The page will now reload."
        );
        setTimeout(() => location.reload(), 1500);
      } catch (error) {
  ...
  const getExportData = () => {
    if (typeof browser !== "undefined" && browser.storage) {
      ...
      return browser.storage.local
        .get(["tiles", "customBackground"])
        .then((result) => {
          return {
            version: 2,
            type: "full",
            tiles:
              result.tiles || JSON.parse(localStorage.getItem("tiles") || "[]"),
            settings: {
          ...
          switch (importedData.type) {
            case "full": // Full backup (settings + links)
              if (importedData.settings) {
                ...
                if (importedData.settings.tilePlacement) {
                  localStorage.setItem(
                    "tilePlacement",
                    importedData.settings.tilePlacement
                  );
                }

                // Persist remaining simple settings
                Object.entries(importedData.settings).forEach(
                  ([key, value]) => {
                ...
                if (importedData.settings.clockSize) {
                  try {
                    document.documentElement.style.setProperty(
                      "--clock-font-size",
                      importedData.settings.clockSize + "px"
                    );
                  } catch (e) { }
                }
                if (importedData.settings.clockPosition && newDigitalClock) {
                  newDigitalClock.classList.remove(
                    "clock-pos-left",
                    "clock-pos-right"
                  );
                  if (importedData.settings.clockPosition === "right")
                    newDigitalClock.classList.add("clock-pos-right");
                  else newDigitalClock.classList.add("clock-pos-left");
                }
              }
              ...
              if (importedData.tiles) {
                if (typeof browser !== "undefined" && browser.storage) {
                  importPromises.push(
                    browser.storage.local.set({ tiles: importedData.tiles })
                  );
                }
                localStorage.setItem(
                  "tiles",
                  JSON.stringify(importedData.tiles)
                );
              }
              ...
              if (importedData.stickyNotes) {
                ...
                localStorage.setItem(
                  "stickyNotes",
                  JSON.stringify(importedData.stickyNotes)
                );
              }
              break;
            ...
            case "settings": // Settings only
              if (importedData.data) {
                ...
                if (importedData.data.tileBorderWidth) {
                  localStorage.setItem(
                    "tileBorderWidth",
                    importedData.data.tileBorderWidth
                  );
                  document.documentElement.style.setProperty(
                    "--tile-border-width",
                    importedData.data.tileBorderWidth
                ...
                if (importedData.data.tilePlacement) {
                  localStorage.setItem(
                    "tilePlacement",
                    importedData.data.tilePlacement
                  );
                }
                ...
                if (importedData.data.clockSize) {
                  try {
                    document.documentElement.style.setProperty(
                      "--clock-font-size",
                      importedData.data.clockSize + "px"
                    );
                  } catch (e) { }
                }
            ...
            case "links": // Links & Notes
              if (importedData.data && Array.isArray(importedData.data)) {
                if (typeof browser !== "undefined" && browser.storage) {
                  importPromises.push(
                    browser.storage.local.set({ tiles: importedData.data })
                  );
                }
                localStorage.setItem(
                  "tiles",
                  JSON.stringify(importedData.data)
                );
              }
              if (importedData.stickyNotes) {
                if (typeof browser !== "undefined" && browser.storage) {
          ...
          Promise.all(importPromises)
            .then(async () => {
              ...
              try {
                ...
                if (importedData.type === "full" && importedData.settings)
                  settingsObj = importedData.settings;
                if (importedData.type === "settings" && importedData.data)
                  settingsObj = importedData.data;
                ...
                if (
                  ...
                  if (skippedKeys.length)
                    console.warn(
                      "Skipping large import keys for browser.storage:",
                      skippedKeys
                    );
                  ...
                  const msg =
                    err && err.message
                      ...
                      : typeof err === "string"
                        ...
                        : JSON.stringify(err);
                  console.warn(
                    "Failed to persist import settings to browser.storage:",
                    msg
                  );
                } catch (e) {
                  console.warn(
                    "Failed to persist import settings to browser.storage: (unserializable error)"
                  );
                }

                // Fallback: persist simple settings into localStorage where possible
                if (
                  typeof settingsObj === "undefined" ||
                  settingsObj === null
                ) {
                  ...
                  try {
                    Object.entries(settingsObj).forEach(([k, v]) => {
                      ...
                      try {
                        ...
                        console.warn(
                          "Failed to persist import setting to localStorage for key",
                          k,
                    ...
                    console.error(
                      "Fallback persistence to localStorage failed:",
  ...
  return new Promise(async (resolve) => {
    ...
    if (typeof browser !== "undefined" && browser.storage) {
      try {
        ...
        console.warn(
          "Failed to save customBackground to browser.storage:",
          err
        );
    ...
    showCustomAlert(
      "Imported background is too large to persist. It will be used for this session only."
    );
    resolve();
  });
}
...
document.body.addEventListener("contextmenu", (e) => {
  const clickedOnTileOrAdd =
    e.target.closest(".tile") || e.target.closest(".add-tile");
  const clickedOnContextMenu = e.target.closest(".context-menu");
  const clickedOnModal =
    e.target.closest(".modal") || e.target.closest(".folder-bubble"); // Include folder-bubble here
...
function openExtensionsPage() {
  ...
  try {
    ...
    console.warn("Unable to open extensions page:", e);
  }
}
  ...
  if (!hasSeenWelcome) {
    ...
    welcomeConfirmBtn.addEventListener("click", () => {
      ...
      try {
        ...
        /* ignore if function/modal missing */
      }
    });
  }
}
...
// Sticky Notes Logic moved to sticky-notes.js
