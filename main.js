...
function openFolder(folder, index) {
  ...
  // Apply folder-specific color to the folder bubble so it matches the folder tile exactly
  if (folder.colorHex) {
    folderBubble.style.background = hexToRgbaAlpha(folder.colorHex, TILE_BG_ALPHA);
    folderBubble.style.borderColor = hexToRgbaAlpha(folder.colorHex, TILE_BORDER_ALPHA);
    folderBubble.style.backdropFilter = 'none';
  } else {
    folderBubble.style.background = '';
    folderBubble.style.borderColor = '';
  ...
  folderBubble.appendChild(folderLinksContainer);
...
if (folderColorInput) {
  folderColorInput.addEventListener('input', (e) => {
    ...
    if (activeFolder && folderEditIndex !== null && activeFolder.index === folderEditIndex) {
      folderBubble.style.background = bg;
      folderBubble.style.borderColor = border;
      folderBubble.style.backdropFilter = 'none';
      folderBubble.querySelectorAll('.tile').forEach(t => {
...
resetAllLinksOnlyBtn.addEventListener('click', () => {
  showCustomConfirm('Are you sure you want to reset ALL your saved links? Your custom settings (colors, fonts, background, etc.) will remain.', () => {
    localStorage.removeItem('tiles');
    if (typeof browser !== 'undefined' && browser.storage) {
      browser.storage.local.remove('tiles').then(() => {
        showCustomAlert('All links have been reset! The page will now reload.');
...
resetAllSettingsAndLinksBtn.addEventListener('click', () => {
  showCustomConfirm('Are you sure you want to reset ALL settings and tiles to default? This will clear all custom colors, fonts, display options, background, AND all your saved links.', () => {
    localStorage.clear();
    if (typeof browser !== 'undefined' && browser.storage) {
...
resetAllSettingsOnlyBtn.addEventListener('click', () => {
  showCustomConfirm('Are you sure you want to reset ALL settings to default? This will clear all custom colors, fonts, display options, and background, but your saved links will remain.', async () => {
...
exportAllSettingsAndLinksBtn.addEventListener('click', () => {
  ...
  getExportData().then(data => {
    downloadJSON(data, 'CagantHomepage_FullBackup.json');
...
exportAllSettingsOnlyBtn.addEventListener('click', () => {
  ...
  downloadJSON(settings, 'CagantHomepage_Settings.json');
...
exportAllLinksOnlyBtn.addEventListener('click', () => {
  const getTiles = () => {
    if (typeof browser !== 'undefined' && browser.storage) {
      return browser.storage.local.get('tiles').then(result => {
        return result.tiles || JSON.parse(localStorage.getItem('tiles') || '[]');
      });
    }
    return Promise.resolve(JSON.parse(localStorage.getItem('tiles') || '[]'));
  };
  ...
  getTiles().then(tiles => {
    ...
    downloadJSON(exportData, 'CagantHomepage_Links.json');
    showCustomAlert('Links exported successfully!');
  });
});
...
// Collapsible clock settings
document.addEventListener('DOMContentLoaded', () => {
  const clockSettingsGroup = document.getElementById('clockSettingsGroup');
  if (!clockSettingsGroup) return;

  const content = clockSettingsGroup.querySelector('.collapsible-content');

  // Ensure the element starts closed (animation uses max-height/opacity)
  if (!clockSettingsGroup.classList.contains('open')) {
    content.style.maxHeight = '0px';
    content.style.opacity = '0';
    content.style.paddingTop = '0px';
  }

  // Toggle function that respects interaction elements
  function toggleClockSettings(forceOpen) {
    const shouldOpen = typeof forceOpen === 'boolean' ? forceOpen : !clockSettingsGroup.classList.contains('open');
    if (shouldOpen) {
      clockSettingsGroup.classList.add('open');
      // animate open
  ...
  // Click anywhere on the group should toggle, except when interacting with form controls
  clockSettingsGroup.addEventListener('click', (ev) => {
    const interactiveTags = ['INPUT', 'SELECT', 'TEXTAREA', 'LABEL', 'BUTTON'];
    // If the click target or any ancestor up to the group is an interactive element, don't toggle
    let node = ev.target;
    while (node && node !== clockSettingsGroup) {
  ...
  // Also make the header keyboard-accessible
  const header = clockSettingsGroup.querySelector('.collapsible-header');
  if (header) {
  ...
  // When an input inside the content receives focus, ensure the section stays open
